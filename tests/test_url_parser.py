...
class TestUrlParser(TestCase):
    ...
    def test_remove_user_info_removes_download_url_key(self) -> None:
    ...
    def test_remove_user_info_removes_download_url_key_and_preserves_parameters(self) -> None:
        storage_uri = urlparse(
    ...
    def test_sanitize_filepath_removes_username_and_password(self) -> None:
        storage_uri = urlparse("https://username:password@bucket/path/filename")
        sanitized_uri = url_parser.sanitize_filepath(storage_uri)

        self.assertEqual("https://bucket/path/filename", sanitized_uri)

    def test_sanitize_filepath_does_not_preserves_parameters(self) -> None:
        storage_uri = urlparse("https://username:password@bucket/path/filename?other=parameter")
        sanitized_uri = url_parser.sanitize_filepath(storage_uri)

        self.assertEqual("https://bucket/path/filename", sanitized_uri)

    def test_sanitize_filepath_preserves_port_number(self) -> None:
        storage_uri = urlparse("ftp://username:password@ftp.foo.com:8080/path/filename")
        sanitized_uri = url_parser.sanitize_filepath(storage_uri)
